...
impl<Storage: ComputeStorage> MemoryManagement<Storage> {
    ...
    pub fn cleanup(&mut self, explicit: bool) {
        // PATCH(llm170, 2026-09-01): 스윕 완전 비활성.
        //
        // 원인(exclusive_pool.rs cleanup): 주기 스윕이 pages를 drain한 뒤
        // free 페이지를 storage.dealloc하고 잔여 페이지 인덱스를 update_page로
        // 재작성한다. 단 1개 페이지라도 dealloc되면 이후 페이지 전부의 인덱스가
        // 시프트되며, (a) 보유 중인 핸들의 디스크립터가 조용히 바뀌고
        // (b) 비행 중 런치 바인딩이 "Memory page N doesn't exist"로 즉사하며
        // (c) HIP에서 libamdhip64 GPF(고정 IP 0x3fc4f2)가 발생한다.
        //
        // llm170은 아레나 규칙(ADR-0014)으로 해제 경로가 없어 스윕의
        // 회수 이득이 0이다 — no-op이 순수 이득. 원 패치 위치:
        // self.persistent.cleanup(...); for pool in ... pool.cleanup(...)
        let _ = explicit;
    }
