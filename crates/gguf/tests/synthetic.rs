...
use llm170_gguf::{GgufFile, GgmlType, Value};
use std::io::Write;
...
fn tmp(name: &str) -> PathBuf {
    ...
    p.push(format!("llm170-gguf-test-{name}-{}.gguf", std::process::id()));
    p
}
...
fn roundtrip_v3() {
    ...
    assert_eq!(t.file_range(f.data_offset), Some((f.data_offset, f.data_offset + 1152)));
    assert_eq!(f.tensor_bytes_total(), Some(1152));
    assert_eq!(f.find_tensor("token_embd.weight").unwrap().ty, GgmlType::Q4K);
