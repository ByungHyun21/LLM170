...
fn qwen35_27b_ud_q4_k_xl() {
    let Some(f) = open_if_exists(P27B) else {
        return;
    };
...
fn qwen4exp_flash_next_split1() {
    let Some(f) = open_if_exists(PFLASH) else {
        return;
    };
    ...
    assert_eq!(
        f.arch_kv_u64("expert_shared_feed_forward_length"),
        Some(640),
        "shared expert FFN(구 추정) 확정"
    );
    ...
    assert_eq!(
        f.arch_kv_u64("attention.indexer.top_k"),
        Some(2048),
        "research 문서의 512가 아님 — 실측 2048"
    );
    assert_eq!(
        f.arch_kv("rope.freq_base")
            .and_then(llm170_gguf::Value::as_f64),
    ...
    let cr = f
        .kv("qwen4exp.attention.compress_ratios")
        .and_then(llm170_gguf::Value::as_array)
        .unwrap()
        .1;
    assert_eq!(cr.len(), 48);
    ...
    assert_eq!(
        offs.len(),
        16,
        "PLE 헤드 수 = (ngram-1)×heads_per_ngram = 16"
    );
