...
fn fmt_dims(t: &TensorInfo) -> String {
    let d: Vec<String> = t.ne[..t.n_dims as usize]
        .iter()
        .map(|v| v.to_string())
        .collect();
    format!("[{}]", d.join(", "))
...
pub fn write_dump(
    ...
    if let Some(s) = f.split() {
        writeln!(
            out,
            "split           : part {} of {} (tensors in part: {})",
            s.no, s.count, s.tensors_count
        )?;
    }
    if let Some(total) = f.tensor_bytes_total() {
        writeln!(
            out,
            "tensor_bytes    : {} (data section {} → {}, {} slack)",
            fmt_bytes(total),
            fmt_bytes(f.data_offset),
    ...
    for (name, (bytes, count)) in by_type.iter() {
        ...
        let bpw = f
            ...
            .map(|t| t.ty.bits_per_weight())
            .unwrap_or(0.0);
        writeln!(
            out,
            "  {name:<10} {:>12} {:>7.2}% {:>8} {:>7.3}",
            fmt_bytes(*bytes),
            share,
            count,
            bpw
    ...
    if broken > 0 {
        writeln!(
            out,
            "  (경고: nbytes 계산 불가 텐서 {broken}개 — ne[0] % blck != 0)"
        )?;
    }
